...
impl Layers {
    ...
    /// How many packages set their own budgets. A reader of one number
    /// should be told when it is not one number.
...
pub fn layers(root: &Path) -> Result<Layers, String> {
    ...
    // default, so without this the config would never find itself.
...
///
/// A marker has to OPEN the line it sits on. A real marker is written
/// in a comment, so the line either starts with the marker or starts
/// with the punctuation that opens the comment. Searching the head as
/// one blob instead reads a documentation sentence as a machine's
/// disclaimer: `phoenix/lib/phoenix/endpoint.ex:24` says "generator, an
/// endpoint was automatically generated as part of your application"
/// inside its own `@moduledoc`, which skips 1097 lines of Phoenix's
/// core silently and takes its imports out of the graph with them, so
/// `transports/websocket.ex` and `transports/long_poll.ex` read as
/// depended on by nothing while the endpoint names both. Fifteen more
/// hand-written files go the same way, each one a script PRINTING a
/// header it generates for somebody else:
/// `git/tools/generate-configlist.sh:35`
...
///
/// The line rule is a strict SUBSET of the blob search, so it cannot
/// lose a true positive, and across gold it loses none: 1770 files
/// flagged by the blob search, 1740 by this one, with the 547
/// flash-attention kernels, php-parser's Php7/Php8, netty's protoc
/// output and the .NET `<auto-generated>` designers all still skipped.
///
/// A minifier strips the comment a marker would live in, so a bundle
/// states nothing about itself and the head test cannot see it. Left in
/// the corpus, ten of gold's `.min.js` files measure as though a person
/// had written them, and twelve such files set the percentile ceilings
/// of a 245-file language.
...
/// A bundle: named `.min.js` or `.min.mjs` AND carrying a line no
/// author wrote.
///
...
mod tests {
    ...
    fn a_bundle_is_named_min_js_and_writes_a_line_no_author_would() {
        ...
        // alone deletes both from a Perl suite's inputs.
        assert!(!is_generated(
    ...
    fn a_marker_used_as_prose_does_not_skip_the_file() {
        ...
        // `@moduledoc`. Read as machine-written it costs 1097 lines of
        // Phoenix's core, and the two transport modules the file names
        // then read as depended on by nothing.
        assert!(!is_generated(
