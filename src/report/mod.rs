...
/// How many parameters travelling together make a clump. Three is the
/// smallest group worth naming as a concept; beyond four the wider
/// signature already reports its narrower subsets.
const CLUMP_WIDTHS: [usize; 2] = [3, 4];

/// Example sites shown per clone class: enough to see the shape
/// repeat, and the rest are counted rather than listed.
const CLONE_SITES: usize = 4;

/// Every `k`-element subset of `items`, each in the input's order.
///
/// Generated by advancing an index vector rather than by nesting one
/// loop per element, so the size of the subset is DATA: nesting would
/// write the combinatorics out, four deep for k=4.
fn subsets<T: Copy>(items: &[T], k: usize) -> Vec<Vec<T>> {
    ...
    loop {
        ...
        // The rightmost index with room left to move; when none has,
        // every subset has been produced.
        let Some(p) = (0..k).rev().find(|&p| idx[p] < n - k + p) else {
            return out;
        };
        idx[p] += 1;
        for q in p + 1..k {
...
pub(crate) const P50: f64 = 0.50;
pub(crate) const P90: f64 = 0.90;
...
///
/// It cannot read as an excuse, which is the risk in putting a local
...
/// reads 7, and `_build_v5` reads 101 magic numbers where the next
/// reads 6.
#[derive(Default)]
struct Local {
...
impl Around<'_> {
    ...
    ///
    /// Only a body OVER the budget asks, since every caller is a
    /// violation, so it is always one of the `over`. Saturating anyway,
    /// because a count that goes backwards should read as "none" rather
    /// than wrap to four billion.
...
impl Offender {
    ...
    fn severity(&self) -> Option<f32> {
        ...
        // has both, and testing `hi` first scores a value under the floor
        // against the ceiling it never reached: `comment ratio` at 0%
        // against a 2%-60% band comes out 0x.
        match self.band {
...
/// A normalized subtree hash and everywhere it occurred, with the FIRST
/// site held inline.
///
...
/// only, and on the gold corpus it would carry a 777,638-entry clone map
/// to the end of the scan and drop it unread.
...
/// blank under a green suite. Nine detector families died that way,
/// unnoticed because nothing checked they were reachable. So a read of a
/// disabled accumulator panics where a test can see it, and
/// `every_mode_asks_for_what_it_reads` runs every mode to prove no
/// renderer touches state its mode did not request.
pub struct Gated<T> {
    on: bool,
    name: &'static str,
    value: T,
...
pub struct Agg {
    ...
    pub generated: u32,
    /// Files named utils/helpers/misc/common, the drawers where concepts
    /// go to lose their names (Zen: namespaces are one honking great
    /// idea).
    pub junk_files: u32,
    pub lines: u64,
    pub units: u64,
    ...
    /// run's units reports `params` firing 651x more often on admired
    /// code than on real code.
    ...
    /// comment should say has to be pinned per ROLE (a field's doc is a
    /// phrase and a module header is a page), so the distribution they
    /// are pinned against has to be readable per role first.
    ...
    /// McCabe's meaning: cyclomatic is a minimum test count.
...
impl Agg {
    ...
    /// Retains every violation, as --json and baselines require.
    #[cfg(test)]
    pub fn complete() -> Agg {
        Agg::configured(
            crate::config::Layers::flat(LangBudgets::defaults()),
    ...
    /// none extracts spuriously. Dropping them costs 165 orphans (curl
    ...
    fn absorb(&mut self, facts: &FileFacts) {
        ...
        for u in &facts.units {
            ...
            // and the clone classes already report file-level
            // duplication.
    ...
    fn collect_clumps(&mut self, u: &crate::facts::UnitFacts, path: &str) {
        ...
        for width in CLUMP_WIDTHS {
            for group in subsets(&names, width) {
                add(&group);
            }
        }
    }
    ...
    ///
    /// On a mixed tree the metrics that fire most are the ones
    /// calibrated per language, so the rows a reader most needs a
    /// number for are the rows with no single band: twelve of them on
    /// the reference tree, `cognitive`, `cyclomatic`, `length` and
    /// `live span` among them.
...
/// above a 34x `live span` because token hygiene is rung 0.
///
/// One entry is one BODY, not one finding: the body-shape metrics move
/// together, so a row per metric names a different function five times
/// for one fact and leaves the functions breaking six budgets each
/// unnamed.
...
/// once, so the import graph is analysed once.
struct Page {
    ink: ink::Ink,
    trim: usize,
    costs: Costs,
...
struct Carried<'a> {
    ...
    /// before one clearing four) puts a body 2x past on five budgets
    /// above the body 13x past on four, and drops the furthest-out body
    /// in the reference tree to tenth. Breadth is worth showing and is;
    ...
    /// and a list naming all five says one thing five times.
    alike: usize,
}

impl Carried<'_> {
    /// What this entry claims: which budgets, at which values.
...
/// 101>11` are not the same size, so a fixed count of four prints a
...
/// then every one of them. `4697x` rather than `4697.4x`, since a tenth
/// of a multiple has never changed anyone's mind about which function
/// to open.
...
/// On the reference tree that discards `config.py` carrying the
/// report's second-worst body while 17 of the tree's 39 modules import
/// it, both facts known and neither on the same line.
struct Costs {
...
/// worst. Every other fact on the line is about the global rule.
///
...
/// ranking looks, since `main`, `<module>` and `generate_spice` have no
/// namesake by construction, and that is where the second clause has
/// the most to say.
...
///
/// The per-file cap holds: one generated lookup table carrying 51,671
/// magic numbers would take two of six slots and say the same thing
/// twice. There is no per-METRIC cap, which would force five rows to
/// name five bodies for one fact.
///
/// A finding a zero budget leaves unrankable stays out: "121 times a
/// budget of none" is a count wearing a ratio's clothes, and the policy
/// section names those per metric. So the count on an entry is of gates
/// that have a distance, not of every gate the body trips.
...
/// counting only the rankable ones reports 8,866 gates on a tree that
/// fails CI on 12,597.
...
/// Findings a budget of zero leaves unrankable, per metric, worst
/// first. Their distance from the budget is a bare count, not a
/// multiple, and letting a count share a ranked list with real
/// multiples puts `suppressions 121` above `cognitive 262>18`.
///
...
/// Only budgets pinned to the corpus. The rows are sorted by violation
/// rate, and unpinned budgets would head the table with compiled-in
/// defaults, which a section comparing against admired code cannot show.
...
/// complex unit asserts" 89.6%, so the verdict is a rate beside gold's
/// rate, not thousands of findings a reader learns to scroll past.
...
/// What this run measured, and everything it declined to measure: the
/// one line a reader sees before any finding.
...
/// Units where several independent facts land at once, worst first, at
/// rung 7: facts that are worse together than apart. The short report
/// prints only how many there are; the long one prints them.
///
...
/// Modules nothing depends on, as a share of the population that could
/// have had a dependent, plus how many could not.
...
fn render_distributions(agg: &mut Agg, out: &mut String) {
    ...
    // 0%-63% sh` pushes every row of `<=0.` thirty characters wide to
...
pub fn render_brief(agg: &mut Agg) -> String {
    ...
    // few times; the rate says which failure is characteristic. The
    // budget is printed because the direction is not always down:
    // `test asserts >=1` is violated by having too FEW, and a header
    // saying "over budget" would be a lie about that row.
...
/// score: a single figure would trade away what this tool knows, which
/// is what kind of claim each rung can support.
...
/// legitimately differ, since C's define-before-use heritage reads
/// bottom-up by design, and the split is itself the finding.
...
/// (Rust types are Pascal and its functions are snake), so the number
/// is shown next to the names rather than judged.
...
/// Does a clump already kept fully explain this one? Only if every
/// parameter here travels inside it AND always together, and equal
/// counts say so.
fn subsumed(names: &[&str], count: u32, kept: &[SelectedClump]) -> bool {
    let inside = |k: &SelectedClump| names.iter().all(|n| k.names.iter().any(|kn| kn == n));
    kept.iter().any(|k| k.count == count && inside(k))
}
...
pub(super) fn select_clumps(agg: &Agg) -> Vec<SelectedClump> {
    ...
    for (names, c) in all {
        if subsumed(&names, c.count, &kept) {
            continue;
        }
...
/// Name association, not coverage, and the section heading says so.
...
/// place at once. Reporting one number for both flatters or damns a
/// codebase depending only on how much of it is tests.
...
/// of an already-selected clone suppressed, so each class is one distinct
...
fn render_clones(agg: &mut Agg, top: usize, out: &mut String) {
    ...
    for class in kept.iter().take(top) {
        ...
        for s in class.sites.iter().take(CLONE_SITES) {
            let _ = writeln!(out, "      {}:{}-{}", s.path, s.line, s.end_line);
        }
        if class.sites.len() > CLONE_SITES {
            let _ = writeln!(
                out,
                "      ... and {} more",
                class.sites.len() - CLONE_SITES
            );
        }
        let _ = writeln!(
            out,
...
/// Named refactorings for what this unit VIOLATES: a suggestion on a
/// metric that is within budget is noise dressed as advice.
...
/// The value at quantile `q` of a sorted distribution. Calibration
/// shares it, so budgets are pinned to the same arithmetic the report
/// prints.
...
mod tests {
    ...
    /// and one clone class, so both tie-break paths get exercised.
    const TWIN: &str = "def NAME(xs):\n    t = 0\n    for x in xs:\n        if x > 0:\n            if x > 1:\n                if x > 2:\n                    if x > 3:\n                        if x > 4:\n                            t += x\n    return t\n";

    /// The per-language denominators are only worth having if they add
    /// up to the pooled totals the report prints: a language row that
    /// silently lost a unit would produce the kind of inflated rate it
    /// exists to prevent. Two languages and a merge, because the rows
    /// are accumulated per rayon worker and folded together afterwards.
    #[test]
    ...
    fn every_mode_asks_for_what_it_reads() {
        ...
        // halves have to agree, and this test checks that agreement.
    ...
    fn repeated_dispatch_needs_the_same_label_set_three_times() {
        const HANDLER: &str = "def h_{}(kind):\n    match kind:\n        case 'a':\n            pass\n        case 'b':\n            pass\n        case _:\n            pass\n";
        let m = |x: &str| HANDLER.replace("{}", x);
        let mut agg = Agg::new();
        let two = m("one") + &m("two");
        agg.add_file(&facts("a.py", &two));
    ...
    /// The ranking's rule: a unit far outside its budget must outrank
    /// one barely past it, even though the second carries the far
    ...
    /// name that side. Formatting it by editing the band's label produces
    /// `0%2%-60%`, and scoring it against the ceiling it never approached
    /// produces a severity of zero.
    #[test]
    ...
    /// capped at one row per metric spends its five rows naming five
    /// bodies for that one fact. The grouping has to keep three claims
    /// true: a body appears ONCE however many metrics it trips, the
    /// entry names all of them, and the ranking is still by distance.
    /// A body 2x past on five budgets is not more urgent than one 13x
    /// past on four, which is what ranking by the count produced on the
    /// reference tree.
    #[test]
    ...
    /// 35>2`, and with no per-metric cap they fill the gold Lua
    /// corpus's top five with one claim stated five times, the failure
    /// the grouping prevents arriving from the other direction. A body
    /// whose claim is already on the list is collapsed into it, and the
    /// entry says how many it stands for so nothing goes quiet. The
    /// numbers must match to the digit: three meridian bodies over the
    /// same three budgets at different values are three functions to
    /// open.
    #[test]
    ...
    /// The rows a reader most needs a number for are the rows with no
    /// single band: the metrics that fire most are the ones calibrated
    /// per language. Both expectations are pinned to `calibration.toml`
    /// on purpose: if a recalibration moves `depth` or `length` this
    /// test says so, the way the README's numbers are pinned.
    #[test]
    fn a_mixed_run_names_the_budget_of_every_language_it_judged_by() {
        ...
        // Most files first, and languages sharing a band named
        // together: the label is a budget, not a list of languages.
    ...
    /// reported apart from the ranked ones. Mixing them lets a count of 121
